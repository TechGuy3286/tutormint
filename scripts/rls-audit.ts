...
const PUBLIC_READ: Record<string, string> = {
  ...
  job_titles: 'curated Job Type titles, rendered in every Job Type picker (migration 77)',
...
const NO_POLICY_OK: Record<string, string> = {
  ...
  admin_backup_codes:
    'staff two-factor backup codes (PR49), hashed: written and consumed by the server only, ' +
    'never read by a client — the phone_otps pattern.',
  match_email_sent:

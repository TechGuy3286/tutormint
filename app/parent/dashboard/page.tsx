...
export default function ParentPortalDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] font-sans text-[#161616] flex flex-col justify-between">
      ...
      {/* Main Browse Marketplace (Shown immediately without forcing login!) */}
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8 flex-1 w-full">
        {/* Audience Selector & Dynamic Messaging */}
        ...
        {/* If Logged In, Show Posted Jobs Section */}
        {parent && (
          <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4 shadow-sm">
            <h2 className="text-base font-extrabold">Your Posted Requirements ({jobs.length})</h2>
            {jobs.length === 0 ? (
              <p className="text-xs text-gray-400">You haven&apos;t posted any job requirements yet.</p>
            ) : (
              <div className="space-y-3">
                {jobs.map((j) => (
                  <div key={j._id} className="p-4 bg-gray-50 rounded-xl flex justify-between items-center text-xs">
                    <div>
                      <strong className="text-sm block">{j.title}</strong>
                      <span className="text-gray-500">Budget: {j.budget} • City: {j.city}</span>
                    </div>
                    <span className="px-3 py-1 bg-emerald-50 text-emerald-800 font-bold rounded-full">🟢 Active</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* LOGIN / SIGNUP MODAL (Triggered only when taking action) */}
      {loginModalOpen && (

...
export default async function ManualPayPage({ params }: { params: Promise<{ ref: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] px-4 py-6 text-[#334155] sm:px-6 sm:py-8">
      <div className="mx-auto max-w-lg space-y-4">
        <header className="space-y-1">
          <h1 className="text-xl font-black text-[#0F172A] sm:text-2xl">Complete your transfer</h1>
        ...
        <section className="space-y-3 rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
          <h2 className="text-sm font-black text-[#0F172A]">1. Send the amount</h2>

          {methods.length === 0 ? (
            <p className="rounded-xl bg-amber-50 p-3 text-xs leading-relaxed text-[#92400E]">
          ...
          <p className="rounded-xl bg-[#F8FAFC] p-3 text-[11px] leading-relaxed">
            Quote this reference in your transfer so we can match it:
            <span className="mt-1 block font-mono text-xs font-bold text-[#0F172A]">
        ...
        <section className="space-y-3 rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
          <h2 className="text-sm font-black text-[#0F172A]">2. Send us the receipt</h2>
        ...
        <p className="flex items-start gap-2 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-[11px] leading-relaxed text-[#92400E]">
        ...
        <Link
          href={packagesHref}
          className="inline-flex min-h-[44px] w-full items-center justify-center rounded-xl border border-gray-200 bg-white px-5 text-xs font-bold text-[#334155]"
        >
          Back to packages
        </Link>
      </div>
...
function Detail({ label, value, mono }: { label: string; value: string | null; mono?: boolean }) {
  ...
  return (
    <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-gray-100 pb-2">
      ...
      <dd className={`text-right font-semibold text-[#0F172A] ${mono ? 'font-mono' : ''}`}>
        {value}
      </dd>
    </div>
  )

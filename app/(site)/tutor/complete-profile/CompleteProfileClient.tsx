...
//
// SERVER/CLIENT SPLIT. page.tsx is the server component; it reads the support
// contact (app_settings + env, never hardcoded) and passes it in, so the Mobile
// step can offer the SAME honest WhatsApp/email fallback /verify-phone has —
// a tutor who cannot receive a code must never be left at a dead end mid-flow.

/** The support fallback, resolved on the server (see page.tsx). */
export type SupportInfo = { waHref: string | null; email: string | null }
...
function CompleteProfileInner({ support }: { support: SupportInfo }) {
    ...
    <main className="min-h-screen bg-tm-bg pb-28 sm:pb-10 text-slate-700">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        <header className="space-y-3">
          ...
          <p className="text-[11px] text-gray-500">A complete profile ranks you higher when parents search.</p>
        </header>
          ...
          {STEPS.map((label, i) => (
            <button
              ...
              className={`min-h-[44px] whitespace-nowrap px-3 py-2 rounded-xl text-[11px] font-bold border transition-colors ${
                step === i + 1 ? 'bg-tm-navy text-white border-tm-navy' : 'bg-white text-slate-700 border-gray-200 hover:bg-gray-50'
              }`}
            >
              {i + 1}. {label}
            </button>
            ...
            <div className="space-y-3" id="phone">
              {phoneVerified ? (
                <>
                  ...
                  {/* A tutor who signed up with a mobile verified it at the phone
                      gate; phone_verified_at already satisfies this step, so it is
                      shown as done and never asks them to verify a second time. */}
                  <p className="text-[11px] leading-relaxed text-gray-500">
                    {phone ? <>Your number <span className="font-semibold text-tm-navy">{phone}</span> is verified — nothing more to do here. </> : null}
                    This counts toward your completion. To change your number, update it in Settings.
                  </p>
                </>
              ) : (
                <>
                  ...
                  {/* Not a dead end. The SAME honest fallback /verify-phone has
                      (owner, Part 6): until a real provider lands, a tutor who
                      cannot receive a code can reach a human instead of being
                      stuck mid-completion. Only shown when a channel is actually
                      configured. */}
                  {(support.waHref || support.email) && (
                    <div className="space-y-2 rounded-2xl border border-gray-200 bg-tm-bg p-4">
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row">
                        ...
                        {support.email && (
                          <a
                            href={`mailto:${support.email}?subject=${encodeURIComponent('Cannot verify my TutorMint mobile number')}`}
        ...
        {/* The actual missing items, always visible — not a bare count behind a
            disclosure. Same source as the percentage (calculateTutorCompletion),
            so the list and the number can never disagree; each links to its step. */}
        {localCompletion.missing.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-2xl p-4 space-y-2">
            <p className="text-xs font-black text-tm-navy">
              Still to do ({localCompletion.missing.length} of {localCompletion.items.length})
            </p>
              ...
              {localCompletion.missing.map((m) => (
                <li key={m.key}>
                  <button
                    ...
                    className="flex w-full items-center gap-2 text-left text-xs text-slate-700 hover:text-tm-red min-h-[36px]"
                  >
                    <span aria-hidden className="text-tm-red">•</span>
                    <span className="flex-1">{m.label}</span>
                    <span className="text-[11px] font-semibold text-gray-500">Step {m.step} ›</span>
                  </button>
                </li>
              ))}
            </ul>
...
// Send code: a secondary, brand navy action (red is reserved for the primary
// Verify / Save & continue). Was hard-coded to tm-black — off-palette, the same
// bug the EmptyState CTA had.
const btnNavy =
  'inline-flex w-full min-h-[44px] items-center justify-center gap-1.5 py-3 bg-tm-navy hover:bg-tm-navy-hover text-white font-bold text-xs rounded-xl disabled:opacity-40 transition-colors'
const btnRed =
...
export default function CompleteProfileClient({ support }: { support: SupportInfo }) {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-xs font-bold text-gray-500">Loading…</div>}>
      <CompleteProfileInner support={support} />
    </Suspense>
  )
}

        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredTutors.map((tutor) => {
              ...
              const cardProps = {
                id: tutorId,
                slug: tutor.slug || tutorId,
                fullName: tutor.full_name || tutor.name || 'Tutor',
                avatarUrl: tutor.avatar_url || tutor.profile_image || tutor.image || '/placeholder-avatar.png',
                rating: Number(tutor.rating) || 5.0,
                reviewsCount: Number(tutor.reviews_count || tutor.reviewCount) || 0,
                subjects: Array.isArray(tutor.subjects) 
                  ? tutor.subjects 
                  : (typeof tutor.subjects === 'string' ? tutor.subjects.split(',').map((s: string) => s.trim()) : [tutor.specialty || tutor.title || 'General Tuition']),
                experience: tutor.experience || tutor.degree || 'Experienced Tutor',
                area: tutor.area || 'All Areas',
                city: tutor.city || 'Pakistan',
                isVerified: Boolean(tutor.is_verified),
                isVeteran: Boolean(tutor.is_veteran),
                isPremium: Boolean(tutor.is_premium),
                isShortlisted: false,
                onShortlist: () => console.log('Shortlist:', tutorId),
                onDemo: () => console.log('Demo:', tutorId),
                onSendMessage: () => console.log('Message:', tutorId)
              }

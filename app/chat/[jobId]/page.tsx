...
export default function ChatRoomPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  const fetchJobAndMessages = async () => {
    try {
      // Fetch Job Details by Transaction ID
      ...
      // Fetch Chat Messages for this Job ID
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-sm font-bold">Loading secure conversation...</div>
  }
      ...
      <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 bg-emerald-500 text-slate-900 text-[10px] font-mono font-bold rounded-full uppercase">
              Sticky Tx ID: {jobId}
            </span>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 h-[450px] flex flex-col justify-between shadow-sm">
        <div className="overflow-y-auto space-y-4 pr-2">
          {messages.length === 0 ? (
            ...
            messages.map((msg, idx) => (
              <div key={idx} className="p-3 bg-gray-50 border border-gray-100 rounded-xl max-w-lg space-y-1">
                <p className="text-xs text-slate-900">{msg.message}</p>
                <span className="text-[10px] text-gray-400 block">{new Date(msg.created_at).toLocaleTimeString()}</span>
              </div>
            ))
          )}
        </div>
        ...
        <form onSubmit={handleSendMessage} className="flex gap-3 pt-4 border-t border-gray-100">
          <input
            ...
            className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:bg-white focus:ring-2 focus:ring-emerald-500"
          />
          <button
            type="submit"

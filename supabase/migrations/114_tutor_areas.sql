-- 114_tutor_areas.sql (PR68)
--
-- Tutors can serve MULTIPLE areas. tutor_areas is the list; tutor_profiles.area is
-- kept in sync with the FIRST area by a trigger, so the listing rule and anything
-- reading the single column keep working. ADDITIVE — code deploys BEFORE this.
--
-- Matching change is minimal and safe (PR68 §4, same method as PR67): the two
-- directory views and rank_tutors gain an `areas` array; rank_tutors' location
-- score now matches on ANY of a tutor's areas; ranking order and listing rules
-- are otherwise unchanged. Validated in a BEGIN … ROLLBACK before the real apply.

-- 1. the list
create table if not exists tutor_areas (
  id uuid primary key default gen_random_uuid(),
  tutor_id uuid not null references tutor_profiles(id) on delete cascade,
  city text,
  area text not null,
  created_at timestamptz not null default now(),
  unique (tutor_id, area)
);
create index if not exists tutor_areas_tutor_idx on tutor_areas(tutor_id);

-- RLS: a tutor reads/writes only their own rows. No anon/authenticated grant —
-- public reads come only through the owner-rights views / SECURITY DEFINER
-- function below (which read tutor_areas as the owner, bypassing RLS).
alter table tutor_areas enable row level security;
drop policy if exists tutor_areas_self on tutor_areas;
create policy tutor_areas_self on tutor_areas for all to authenticated
  using (tutor_id = auth.uid()) with check (tutor_id = auth.uid());

-- 2. backfill: each tutor's existing single area becomes their first entry
insert into tutor_areas (tutor_id, city, area)
select tp.id, tp.city, btrim(tp.area)
from tutor_profiles tp
where tp.area is not null and btrim(tp.area) <> ''
on conflict (tutor_id, area) do nothing;

-- 3. keep tutor_profiles.area = the FIRST (oldest) area
create or replace function sync_tutor_first_area() returns trigger as $$
declare tid uuid;
begin
  tid := coalesce(new.tutor_id, old.tutor_id);
  update tutor_profiles tp
     set area = (select ta.area from tutor_areas ta where ta.tutor_id = tid order by ta.created_at, ta.id limit 1)
   where tp.id = tid;
  return null;
end;
$$ language plpgsql;
drop trigger if exists sync_tutor_first_area_trg on tutor_areas;
create trigger sync_tutor_first_area_trg after insert or delete or update on tutor_areas
  for each row execute function sync_tutor_first_area();

-- 4. views gain the areas array (owner-rights; correlated subquery)
...
-- 5. rank_tutors: location score matches ANY area; areas in the output
...
AS $function$
  ...
  eligible as (
    select
      d.id, d.slug, d.full_name, d.headline, d.avatar_url, d.city, d.area, d.areas,
...
-- 6. tutor_public_page returns the areas array too
drop function if exists tutor_public_page(text);
CREATE OR REPLACE FUNCTION public.tutor_public_page(p_slug text)
 RETURNS TABLE(id uuid, slug text, full_name text, headline text, bio text, avatar_url text, city text, area text, areas text[], teaching_mode text, job_types text[], online_platforms text[], gender text, hourly_rate_pkr integer, fee_min_pkr integer, fee_max_pkr integer, experience_years integer, degrees text[], video_youtube_id text, video_status text, rating_avg numeric, rating_count integer, created_at timestamp with time zone, plan_code text, subjects jsonb, slots jsonb, reviews jsonb, degree_documents jsonb)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  select
    d.id, d.slug, d.full_name, d.headline, d.bio, d.avatar_url, d.city, d.area, d.areas,

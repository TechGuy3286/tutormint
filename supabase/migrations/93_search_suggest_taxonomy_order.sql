-- 93_search_suggest_taxonomy_order.sql (owner PR14 §5.1)
--
-- Subject suggestions are ordered by MATCH SCORE then the curriculum's own
-- sequence — the levels of a resolved subject read Pre-Nursery → university, in
-- taxonomy order, NOT by open-tuition count (this reverses migration 91). The
-- "<subject> · all levels" entry is prepended in the API route, which then keeps
-- the individual levels in the order this function returns them. CREATE OR
-- REPLACE with ONE change from migration 91: the `open_counts` CTE and its join
-- are removed and the `subjects` ORDER BY is `sc desc, cat_order, lvl_order`.
-- Signature, grants and SECURITY DEFINER unchanged.
...
subjects as (
  ...
  from tax_scored ts
  where ts.sc > 0.3
  -- Ordered by MATCH SCORE, then the curriculum's own sequence (owner PR14 §5.1):
  -- the levels of a resolved subject all score the same, so they come back in
  -- taxonomy order (Pre-Nursery → university), never by count.

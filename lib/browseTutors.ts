...
export async function tutorCardBySlug(slug: string): Promise<TutorCardData | null> {
  ...
  const { data } = await supabase
    .from('tutor_directory')
    .select(
      'id, slug, full_name, headline, avatar_url, city, area, areas, teaching_mode, job_types, hourly_rate_pkr, fee_min_pkr, fee_max_pkr, experience_years, rating_avg, rating_count, degrees, verified_fee_paid_at',
    )
    .eq('slug', slug)
    .maybeSingle()
  if (!data) return null
